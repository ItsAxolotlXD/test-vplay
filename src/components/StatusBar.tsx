  ...
  return (
    <>
      ...
      >
        ...
        {/* 2. MIDDLE VERTICAL DOCK: 4 KÊNH XEM GẦN ĐÂY */}
        <div className="flex flex-col items-center pointer-events-auto my-auto py-1">
          <div
            id="status-bar-recent-channels-dock"
            className="p-1 sm:p-1.5 rounded-2xl sm:rounded-[22px] bg-white/30 dark:bg-white/20 backdrop-blur-2xl border border-white/20 shadow-xl shadow-black/25 flex flex-col items-center gap-1.5 sm:gap-2 relative overflow-hidden"
          >
            {/* Top & Bottom white border with horizontal fade */}
            <svg
              className="shiny-rim-svg absolute inset-0 w-full h-full pointer-events-none rounded-2xl sm:rounded-[22px]"
              viewBox="0 0 52 188"
              preserveAspectRatio="none"
              fill="none"
            >
              <defs>
                <linearGradient id="dock-rim-grad" x1="0%" y1="50%" x2="100%" y2="50%">
                  <stop offset="0%" stopColor="#ffffff" stopOpacity="0" />
              ...
              <rect
                ...
                height="186.4"
                rx="21.2"
                stroke="url(#dock-rim-grad)"
                strokeWidth="1.4"
              />
            </svg>

            {/* Subtle top indicator bar */}
            <div className="w-3.5 h-0.5 rounded-full bg-white/40 my-0.5" title="Kênh xem gần đây" />

            {/* 4 Recent Channel Logos */}
            {recentChannels.map((ch, idx) => {
              const isCurrent = currentChannel?.id === ch.id || currentChannel?.slug === ch.slug;
              return (
                <div key={ch.id || idx} className="relative group flex items-center justify-center">
                  <motion.button
                    type="button"
                    whileHover={{ scale: 1.15 }}
                    whileTap={{ scale: 0.92 }}
                    onClick={() => handleSelectRecentChannel(ch)}
                    className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full relative overflow-hidden flex items-center justify-center p-1 cursor-pointer transition-all duration-200 ${
                      isCurrent
                        ? 'bg-black/80 ring-2 ring-emerald-400 shadow-[0_0_12px_rgba(52,211,153,0.5)]'
                        : 'bg-black/50 hover:bg-black/75 border border-white/20 hover:border-white/50'
                    }`}
                    title={`${ch.name} - ${ch.category || 'Live TV'}`}
                    aria-label={`Xem kênh ${ch.name}`}
                  >
                    {ch.logo ? (
                      <img
                        src={ch.logo}
                      ...
                      <span className="text-[10px] sm:text-[11px] font-bold text-white font-mono uppercase">
                        {(ch.shortName || ch.name).substring(0, 3)}
                      </span>
                    )}
                  ...
                  {/* Hover Tooltip popping to the left */}
                  <div className="absolute right-full mr-3 top-1/2 -translate-y-1/2 px-2.5 py-1.5 rounded-xl bg-zinc-900/95 dark:bg-black/95 text-white text-[11px] font-medium whitespace-nowrap shadow-xl border border-white/15 backdrop-blur-md opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-150 transform translate-x-1 group-hover:translate-x-0 z-50 flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-cyan-400" />
                    <span>{ch.name}</span>
                    {isCurrent && <span className="text-[10px] text-emerald-400 font-semibold">(Đang phát)</span>}
                  </div>
                </div>
              );
            })}
