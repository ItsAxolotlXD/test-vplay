...
import { Volume2, VolumeX, FastForward, Play, Loader2 } from 'lucide-react';
...
const DEFAULT_LOCAL_VIDEO = '/intro-video.mp4';
const FALLBACK_PROXY_VIDEO = '/api/intro-video';
const WIKIA_ORIGINAL_VIDEO = 'https://static.wikia.nocookie.net/ep-deo/images/4/4a/5c1imv.mp4/revision/latest?cb=20260924070114';
...
export const StartupVideoIntro: React.FC<StartupVideoIntroProps> = ({
  ...
  // Keyboard shortcut listener (Esc / Space / Enter to skip or toggle)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const attemptPlay = async () => {
      try {
        ...
        // Autoplay with sound blocked by browser policy, try muted
        try {
          video.muted = true;
          setIsMuted(true);
          await video.play();
    ...
    // Safety timeout: in case video never finishes after 30s, automatically proceed
      ...
      {!isFinished && (
        ...
        >
          ...
          <video
            ...
            onEnded={() => {
              // Video finished naturally -> proceed to splash screen
              handleComplete();
            }}
            onError={(e) => {
              console.warn('Video source error encountered:', e);
              // If video couldn't play, fall back after brief moment so user isn't stuck
          ...
          >
            {/* 1. Direct local served by Express streaming route */}
            <source src={DEFAULT_LOCAL_VIDEO} type="video/mp4" />
            {/* 2. Express Range-stream API */}
            <source src={FALLBACK_PROXY_VIDEO} type="video/mp4" />
            {/* 3. Server Video Proxy with Hotlink Referer Bypass */}
            <source
              src={`/api/video-proxy?url=${encodeURIComponent(videoUrl || WIKIA_ORIGINAL_VIDEO)}`}
              type="video/mp4"
            />
            {/* 4. Direct Wikia CDN link as last fallback */}
            <source src={videoUrl || WIKIA_ORIGINAL_VIDEO} type="video/mp4" />
          </video>
          ...
          {isBuffering && !showPlayOverlay && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-15">
              <div className="flex items-center gap-3 px-4 py-2 rounded-full bg-black/60 backdrop-blur-md border border-white/10 text-white/80 text-xs font-medium">
                ...
                <span>Đang tải video intro...</span>
              </div>
            </div>
          )}
          ...
          <div className="absolute top-4 right-4 sm:top-6 sm:right-6 z-30 flex items-center gap-2.5 pointer-events-auto">
            ...
            <button
              ...
              title="Bỏ qua video và vào Splash Screen"
            >
              <span>Bỏ qua</span>
          ...
          {/* Bottom Bar: Progress Indicator & Time */}
